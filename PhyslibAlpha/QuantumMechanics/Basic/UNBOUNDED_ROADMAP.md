...
- **Part 1, step 0** (`WOTSpectralMeasure`) — **built**, in `HilbertSpace/Unbounded/{Basic,
  ...
  representation-level half) are already here.
- **Part 1, step 1** (Cayley-transform construction) — **partially built**, in
  `HilbertSpace/Unbounded/Cayley/{Basic,Measure,Certificate,Inverse}.lean`: the scalar Cayley map
  and its inverse, `cayleyPMap`/`cayleyUnitary` (transporting an unbounded self-adjoint operator to
  a bounded unitary), pushing a `WOTSpectralMeasure` through the transform, and the partial inverse
  Cayley operator with its density/self-adjointness theory. **Not yet ported**: the source's
  `CayleySpectralData/{P1,P2}.lean` and `BoundedSelfAdjointData.lean` — the two source files this
  4-file batch doesn't yet cover. Confirmed the source's own `Affil/Concrete.lean` import chain was
  NOT needed (only `WOTSpectralMeasure` and `Physlib`'s unbounded-operator API), matching the
  roadmap's prediction.
- **Part 2, step 1** (abstract `WStarAlgebra` class) — **built**, in `WStarAlgebra/Basic.lean`.
  **Real collision found and fixed**: Mathlib already has its own `WStarAlgebra` (`Mathlib.
  ...
  conjugate-linear) is documented as real, unattempted work, not forced.
- **Part 2, step 2** (trace-pairing surjectivity — first half, the trace-class predicate) —
  **built**, in `HilbertSpace/TraceClass/Basic.lean`: `IsTraceClass`/`traceNorm`/`trace`/
  `HasFiniteMultiplicity` on individual bounded operators, distinguished from `HilbertSpace/
  Trace.lean`'s existing total (finite-dimension-only) trace, with the genuine connective fact
  between the two ported. **Not yet built**: the trace-class *Banach space* `TraceClass H` itself
  (source: `OperatorAlgebra/TraceClass/{GeneralProduct,IdealNorm,PositiveIdeal,...}.lean`, ~3150
  more lines) and trace-pairing surjectivity proper (`WStarAlgebra/TracePairingSurj.lean`, 545
  lines) — both still need dispatching.

## Next steps

- Part 1: `CayleySpectralData/{P1,P2}.lean` (~1705 lines) and `BoundedSelfAdjointData.lean`
  (200 lines), completing step 1, then step 2 (the bounded-unitary spectral theorem itself, pulled
  back through Cayley — the deepest single piece of this whole roadmap).
- Part 2: the `TraceClass H` Banach space, then trace-pairing surjectivity, then the concrete
  `B(H)` `WStarAlgebraStructure` instance tying `WStarAlgebra/Basic.lean` to something real.
