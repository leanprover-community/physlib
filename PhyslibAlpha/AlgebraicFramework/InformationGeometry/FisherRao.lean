...
Robertson--Schrödinger in algebraic uncertainty frameworks.
...
structure OpenSimplex (α : Type*) [Fintype α] where
  /-- The probability weight at each outcome. -/
  val : α → ℝ
...
/-- The Fisher--Rao inner product at $p \in \Delta^\circ(\alpha)$:
    `g_p(u, v) = \sum_{i \in \alpha} \frac{u_i v_i}{p_i}` -/
...
/-- $g_p(u, u) = 0 \iff u = 0$ (positive definiteness). -/
...
/-- Expansion of the Fisher--Rao norm under linear combinations. -/
...
/-- Cauchy--Schwarz for the Fisher--Rao inner product:
    $g_p(u, v)^2 \le g_p(u, u) \cdot g_p(v, v)$. -/
...
/-- Fisher information equals the Fisher--Rao squared norm of the
    derivative vector: $I(\theta) = \|dp/d\theta\|^2_{\mathrm{FR}}$. -/
