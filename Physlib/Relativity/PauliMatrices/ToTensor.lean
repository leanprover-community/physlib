...
/-- Gaussian integer components of the contravariant Pauli four-vector. -/
def pauliContrComponent (mu : Fin 4) (a b : Fin 2) : GaussianInt :=
...
/-- Gaussian integer components of the contravariant conjugate Pauli four-vector. -/
def pauliContrDownComponent (mu : Fin 4) (a b : Fin 2) : GaussianInt :=
  if mu.val = 0 ∧ a.val = b.val then ⟨1, 0⟩ else
  if mu.val = 1 ∧ a.val ≠ b.val then ⟨-1, 0⟩ else
  if mu.val = 2 ∧ a.val = 0 ∧ b.val = 1 then ⟨0, 1⟩ else
  if mu.val = 2 ∧ a.val = 1 ∧ b.val = 0 then ⟨0, -1⟩ else
...
lemma toTensor_eq_ofGaussianInt : σ^^^ = ofGaussianInt (fun b =>
  ...
  repeat rw [basis_eq_ofGaussianInt]
  have hI : I = GaussianInt.toComplex ⟨0, 1⟩ := by simp [GaussianInt.toComplex_def']
  simp only [Fin.isValue, map_sub, map_add, _root_.map_smul, Finsupp.coe_sub, Finsupp.coe_add,
    Finsupp.coe_smul, Pi.sub_apply, Pi.add_apply, ofGaussianInt_basis_repr_apply, Pi.smul_apply,
    smul_eq_mul, hI, Nat.succ_eq_add_one, Nat.reduceAdd]
  simp only [Fin.isValue, ← map_mul, ← map_add, ← map_sub]
...
lemma toTensor_dualLeft_eq_ofGaussianInt :
  ...
  conv_lhs =>
    rw [toTensor_eq_ofGaussianInt, toDualMapAtIndex]
    change crossToSlot 1 0 (by rfl) M (ofGaussianInt _)
    erw [crossToSlot_eq_crossToEnd, crossToEnd]
    simp only [LinearMap.compr₂_apply, LinearMap.comp_apply]
    dsimp only [M]
    rw [dualLeftMetric_eq_ofGaussianInt, prodT_ofGaussianInt_ofGaussianInt, permT_ofGaussianInt,
      contrT_ofGaussianInt, permT_ofGaussianInt, permT_ofGaussianInt]
  -- `congr` would first try `rfl` on the component functions, which is expensive here.
...
lemma toTensor_dualWeyl_eq_ofGaussianInt :
  ...
  rw [toTensor_dualLeft_eq_ofGaussianInt]
  let M : ℂT[.downR, .downR] := εR'
  conv_lhs =>
    rw [toDualMapAtIndex]
    change crossToSlot 2 0 (by rfl) M (ofGaussianInt _)
    erw [crossToSlot_eq_crossToEnd, crossToEnd]
    simp only [LinearMap.compr₂_apply, LinearMap.comp_apply]
    dsimp only [M]
    rw [dualRightMetric_eq_ofGaussianInt, prodT_ofGaussianInt_ofGaussianInt, permT_ofGaussianInt,
      contrT_ofGaussianInt, permT_ofGaussianInt, permT_ofGaussianInt]
  congr
  funext b
  decide +revert +kernel
...
lemma toTensor_dualLorentz_eq_ofGaussianInt :
  ...
  conv_lhs =>
    rw [toTensor_eq_ofGaussianInt, toDualMapAtIndex]
    change crossToSlot 0 0 (by rfl) M (ofGaussianInt _)
    erw [crossToSlot_eq_crossToEnd, crossToEnd]
    simp only [LinearMap.compr₂_apply, LinearMap.comp_apply]
    dsimp only [M]
    rw [coMetric_eq_ofGaussianInt, prodT_ofGaussianInt_ofGaussianInt, permT_ofGaussianInt,
      contrT_ofGaussianInt, permT_ofGaussianInt, permT_ofGaussianInt]
  congr
  funext b
  decide +revert +kernel
...
lemma toTensor_dualLorentzLeft_eq_ofGaussianInt :
    {σ^^^ | τ(μ) τ(α) β}ᵀ = ofGaussianInt (fun b =>
      ∑ x : Fin 2, pauliContrDownComponent (b 0) x (b 2) *
        (if x.val = 0 ∧ (b 1).val = 1 then 1 else
          if (b 1).val = 0 ∧ x.val = 1 then -1 else 0)) := by
  rw [toTensor_dualLorentz_eq_ofGaussianInt]
  let M : ℂT[.downL, .downL] := εL'
  conv_lhs =>
    rw [toDualMapAtIndex]
    change crossToSlot 1 0 (by rfl) M (ofGaussianInt _)
...
lemma toTensor_dualAll_eq_ofGaussianInt :
    {σ^^^ | τ(μ) τ(α) τ(β)}ᵀ =
      ofGaussianInt (fun b => pauliContrComponent (b 0) (b 2) (b 1)) := by
  rw [toTensor_dualLorentzLeft_eq_ofGaussianInt]
  let M : ℂT[.downR, .downR] := εR'
  conv_lhs =>
    rw [toDualMapAtIndex]
    change crossToSlot 2 0 (by rfl) M (ofGaussianInt _)
...
lemma pauliCo_eq_ofGaussianInt : pauliCo = ofGaussianInt (fun b =>
  ...
  conv_lhs =>
    ...
    simp only [coMetric_eq_ofGaussianInt, ofGaussianInt_basis_repr_apply, toTensor_eq_ofGaussianInt]
...
lemma pauliCoDown_eq_ofGaussianInt : pauliCoDown = ofGaussianInt (fun b =>
  ...
  conv_lhs =>
    ...
    rw [prodT_basis_repr_apply]
    rw [contrT_basis_repr_apply]
    simp only [coMetric_eq_ofGaussianInt, ofGaussianInt_basis_repr_apply,
      dualLeftMetric_eq_ofGaussianInt]
    enter [1, 1, 2, y]
    rw [contr_basis_gaussianInt]
    rw [prodT_basis_repr_apply]
    simp only [coMetric_eq_ofGaussianInt, ofGaussianInt_basis_repr_apply, pauliCo_eq_ofGaussianInt,
      dualRightMetric_eq_ofGaussianInt]
    rw [← GaussianInt.toComplex.map_mul]
    rw [← GaussianInt.toComplex.map_mul]
  conv_lhs =>
    enter [2, x]
...
lemma toTensor_dualWeyl_eq_pauliContrDown :
  ...
  rw [toTensor_dualWeyl_eq_ofGaussianInt]
  rw [pauliContrDown_ofGaussianInt, permT_ofGaussianInt]
  congr

/-- Dualizing all three indices of `σ^^^` gives `σ___`. -/
lemma toTensor_dualAll_eq_pauliCoDown :
    ({σ^^^ | τ(μ) τ(α) τ(β) = σ___ | μ β α}ᵀ : Prop) := by
  rw [toTensor_dualAll_eq_ofGaussianInt]
...
lemma pauliDual_eq_pauliCo :
  ...
  rw [toTensor_dualLorentz_eq_ofGaussianInt, pauliCo_eq_ofGaussianInt, permT_ofGaussianInt]
  congr
...
lemma pauliContrDownDual_eq_pauliCoDown :
  ...
  have hDual :
    ...
    conv_lhs =>
      rw [pauliContrDown_ofGaussianInt]
      rw [toDualMapAtIndex]
      change crossToSlot (S := complexLorentzTensor) 0 0 rfl η' (ofGaussianInt _)
      rw [crossToSlot_eq_crossToEnd, crossToEnd]
      simp only [LinearMap.compr₂_apply, LinearMap.comp_apply]
      rw [coMetric_eq_ofGaussianInt]
      rw [prodT_ofGaussianInt_ofGaussianInt, permT_ofGaussianInt, contrT_ofGaussianInt,
        permT_ofGaussianInt, permT_ofGaussianInt]
    conv_rhs =>
      rw [pauliCoDown_eq_ofGaussianInt]
    apply (Tensor.basis _).repr.injective
    ext b
    conv_rhs =>
