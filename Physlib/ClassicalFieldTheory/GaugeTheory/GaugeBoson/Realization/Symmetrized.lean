...
lemma eq_zero_of_forall_sum_smul_pow_eq_zero {n : ℕ} {b : ℕ → B}
  ...
  rwa [Finset.sum_ite_eq (Finset.range n) m (fun j => f (b j)), ite_eq_left hm] at hcoeff
...
lemma mem_of_translationInvariant_single (R : Subalgebra ℂ B) (y : B)
  ...
  | _ n ih =>
    ...
    have hexp : ∀ t : ℝ, Φ t (∑ k ∈ Finset.range (m + 2), r k * y ^ k) =
      ...
      refine ((Finset.sum_congr rfl fun j hj => ?_).trans
        (Finset.sum_subset (Finset.range_subset_range.mpr
          ...
          fun j _ hj => ite_eq_right fun h => hj (Finset.mem_range.mpr (Nat.lt_succ_of_le h))))
    ...
    have hrtop : r (m + 1) = 0 := by
      ...
      rw [ite_eq_right (Nat.succ_ne_zero m), hbdef] at h
      simp only at h
      rwa [Finset.sum_congr rfl fun k hk => show
        ...
        ite_eq_left (Finset.self_mem_range_succ _), Nat.choose_self, Nat.sub_self, pow_zero,
...
lemma coord_shiftFamily (t : Sym (Fin 1 ⊕ Fin 3) (m + 1) × ι → ℝ)
  ...
  classical
  rw [shiftFamily, dite_eq_left (Sym.card_coe (s := ps)), map_sum]
