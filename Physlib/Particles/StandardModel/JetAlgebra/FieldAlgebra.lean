...
The gauge tensor factor is the complexification `ℂ ⊗[ℝ] GaugeJetAlgebra GaugeAlgebra`, while the
gauge
sector's generation theorem is a statement over `ℝ` about `GaugeJetAlgebra GaugeAlgebra` itself.
The
...
noncomputable def includeGaugeReal : (GaugeJetAlgebra GaugeAlgebra) →ₐ[ℝ] JetAlgebra :=
  (AlgHom.restrictScalars ℝ includeGauge).comp
    (Algebra.TensorProduct.includeRight (R := ℝ) (A := ℂ) (B := (GaugeJetAlgebra GaugeAlgebra)))
...
lemma includeFermion_mul_includeHiggs_mul_includeGauge (a : FermionJetAlgebra)
    (b : HiggsJetAlgebra) (c : ℂ ⊗[ℝ] (GaugeJetAlgebra GaugeAlgebra)) :
...
theorem adjoin_generators_eq_top :
  ...
  rw [← Algebra.TensorProduct.adjoin_tmul_eq_top ℂ
    (FermionJetAlgebra ⊗[ℂ] HiggsJetAlgebra) (ℂ ⊗[ℝ] (GaugeJetAlgebra GaugeAlgebra))]
